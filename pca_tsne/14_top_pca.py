...
path_to_log_file = os.path.join(base_save_folder, "by_top","top_log_pca.txt")
original_stdout = sys.stdout
sys.stdout = Logger(path_to_log_file)

print("--------------------------------\n")
print("----- PCA by TOPOLOGY ------\n")
...
# ---------------------------------------------------------------------------------------------------
# PCA TOPOLOGY by HOMOLOGOUS SUPERFAMILY
...
for top_name in df_processed['top_description'].unique():
    ...
    if df_top_numeric.shape[0] > 1 and df_top_numeric.shape[1] > 1:
        ...
        principal_components = pca.fit_transform(df_top_numeric)
        pca_df = pd.DataFrame(data=principal_components, columns=['PC1', 'PC2'], index=df_top_numeric.index)
    else:
        print(f"Skipping PCA for {top_name}: Insufficient data. Shape: {df_top_numeric.shape}")
        continue

    # Add Homologous Superfamily descriptions for plotting
    pca_df['super_description'] = df_top_processed.loc[df_top_numeric.index, 'super_description']

    print(f"\tHomologous Superfamily:", ", ".join(pca_df['super_description'].unique()), "\n")

    explained_variance = pca.explained_variance_ratio_ * 100

    # ---------------------------------------------------------------------------------------------------
    # figure
    num_unique_topologies = len(pca_df['super_description'].unique())
    color_indices = np.linspace(0, 1, num_unique_topologies)
    spectral_colors = plt.cm.Spectral(color_indices)

    # Mapping each Homologous Superfamily to a color
    colour_map = dict(zip(pca_df['super_description'].unique(), spectral_colors))
    ...
    plt.title(f'PCA for {top_name} (Topology) by Homologous Superfamily')
    plt.xlabel(f'PC1: {explained_variance[0]:.2f}%')
    plt.ylabel(f' PC2: {explained_variance[1]:.2f}%')


    plt.savefig(os.path.join(top_save_folder, f"{top_name.replace(' ', '_')}_pca.png"), bbox_inches='tight')
    plt.close()
    ...
    pca_feature_names = df_top_numeric.columns.tolist()
    ...
    for i in range(pca.n_components_):
        ...
        plt.title(f'PCA Component {i+1} Loadings for {top_name}')
        plt.xlabel('Features')
        plt.ylabel('Loading Value')
        plt.tight_layout()
        plt.savefig(os.path.join(top_save_folder, f'pc_{i+1}_loadings_{top_name}.png'))
        plt.close()

    print(f"\tTopology number of structures: {len(pca_df)}", "\n")
    print("\tVariance explained by each component:", explained_variance, "\n")
    print(f"\tAnalysis completed for {top_name}.\n")
